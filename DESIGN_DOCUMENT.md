...
Typical: 30-40 moves
Buffer: 70 moves × 4 bytes = 280 bytes (safe margin)
```
...
Evaluation Stack:    200 bytes (for minimax, 50 nodes × 4 bytes)
...
Complexity: O(n) where n = number of legal moves
Estimated Time: < 5 seconds (40 moves × 100ms evaluation)
```
